...
# NOME DIFERENTE PARA NÃO SOBRESCREVER A RESNET
MODEL_NAME = "mobilenet_v2_rop.keras"
IMAGE_SIZE = (224, 224)
BATCH_SIZE = 32

try:
    print("\n--- INICIANDO EXPERIMENTO: MOBILENET V2 ---")
    ...
    fine_tune_at = 100  # Ajuste para MobileNet
    ...
    print("\n--- Gerando Matriz de Confusão (MobileNetV2) ---")
    predictions = model.predict(test_ds)
    y_pred = (predictions > 0.5).astype(int).flatten()  # Usando 0.5 padrão para comparação justa
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges')  # Laranja para diferenciar do "Campeão"
    plt.title(f'MobileNetV2 (Acurácia: {np.mean(y_true == y_pred):.2%})')
    plt.ylabel('Real')
    plt.xlabel('Predito')
    plt.show()
